...
try:
    ...
    import dataBaseScripts.uclchem as uclchem
...
def UCLChemDataFrames(UCLChemDict, outSpecies, Test=False):
    ...
    UniqueOutSpecies = np.unique(np.asarray([chemForUCLCHEM(x) for x in outSpecies.split()]))
    UniqueOutSpecies = "H H+ " + " ".join(UniqueOutSpecies)
    UCLChemDict["outSpecies"] = len(UniqueOutSpecies.split())
    CurrentPID = os.getpid()
    mcf.FortranQueue.put(("RunUCLCHEM", [UCLChemDict, UniqueOutSpecies.split(), UCLChemDict['points'], parameterArray,
                                  chemicalAbunArray, stepCount, CurrentPID]))
    listOfArrays = RetrieveFortranQueueResults(CurrentPID)
    ...
    points = np.shape(parameterArray)[1]
    UniqueOutSpecies = UniqueOutSpecies.split()[2:]
    if points == 1:
        ...
        physDF['H'] = chemicalAbunArray[:stepCount[0], :, 0]
        physDF['H+'] = chemicalAbunArray[:stepCount[0], :, 1]
        chemDF = pd.DataFrame(data=chemicalAbunArray[:, 0, 2:], columns=UniqueOutSpecies)
    else:
        exit("Currently unable to run with multiple Points")
    return physDF, chemDF


def FortranUCLCHEM(dictionary, outspeciesin, numberpoints, parameterarray, chemicalabunarray, stepcount, CurrentPID):
    with suppress_stdout_stderr():
        uclchem.to_df(dictionary=dictionary, outspeciesin=outspeciesin, numberpoints=numberpoints,
        ...
        for i in UCLChemDF.columns:
            ...
            if 1e5 <= radexDic['cdmol'] <= 1e25:
                for j in ReverseChemForUCLCHEM(i).split(", "):
                    try:
                        radexDic['molfile'] = "./SpectralRadex/radex/data/" + chemDat(j)
...
def runRadex(RadexParamDict, Queue=True):
    ...
    if "_" in dataFrame["QN Upper"].iloc[0] or " " in dataFrame["QN Upper"].iloc[0]:
        for i in range(np.shape(dataFrame)[0]):
            TransitionName = RadexParamDict["molfile"][27:-4] + "_(" + \
...
def saveModel(UCLParamDF, ParamDict, outspecies, DatabaseLocation, CurrentPID):
    UCLParamDF.loc[0, "outSpecies"] = outspecies
    ...
    for i in UCLParamDF.columns.to_list():
        if i not in savedModelsColumns:
            nonSaveColumns += [i]
    UCLCHEMTransitionDF = UCLParamDF[nonSaveColumns].copy()
    UCLParamDF = UCLParamDF.drop(nonSaveColumns, axis=1)
    con = sql.connect(DatabaseLocation, timeout=timeOutForSQL)
    cur = con.cursor()
    ...
    for i in outspecies.split():
        ...
        if len(columnsOfInterest) <= 1000:
            UCLCHEMTransitionDFTemp = UCLCHEMTransitionDF[columnsOfInterest].copy()
            ...
            for k in range(itterations):
                ...
                UCLCHEMTransitionDFTemp = UCLCHEMTransitionDF[columnsThisLoop].copy()
                ...
                con.commit()
    con.close()
    mcf.ResultDict[str(CurrentPID)] = "Complete"
    return None


def retrieveClosestMatches(LinesArray, DistanceMethod, DBlocation):
    ...
    sqlSearch = sqlSearch + ';'
    CurrentPID = os.getpid()
    mcf.Queue.put(("Search", [DBlocation, sqlSearch, CurrentPID]))
    SearchResults = RetrieveQueueResults(CurrentPID)
    return SearchResults.values.T[0]


def retrieveLinesOfEntry(ModelID, LinesOfInterest, DBlocation, ChemsAndLinesDic):
    sqlSearch = None
    FromString = ""
    PreviousDat = ""
    WhereString = ""
...
def retrieveEntry(ID, DBlocation, ModelParameters=False):
    if ModelParameters:
...
def retrieveIntensities(ParameterDict, ChangingParams, DBlocation, transitionFile, outSpecies, Units=None):
    ChangingParamsKeys = list(ChangingParams.keys())
    ChangingParamsValues = list(ChangingParams.values())
    if Units == None and type(outSpecies) != int:
        Units = ["Flux"] * len(outSpecies)
    elif Units == None and type(outSpecies) == int:
        Units = ["Flux"] * outSpecies
    MatchedID = []
    while len(MatchedID) < 1:
        MatchedID = checkAndRetrieveModel(ParameterDict, ChangingParamsKeys, ChangingParamsValues, DBlocation)
    chemicalDatsInterest = [chemDat(x) for x in outSpecies.split()]
    TransitionDF = pd.read_csv(transitionFile, delimiter=",/", engine='python')
    LinesInSql = np.array([])
    for i in range(len(chemicalDatsInterest)):
        Transitions = TransitionDF['Transitions'].loc[TransitionDF['.datFile'] == chemicalDatsInterest[i]].values[0].split(", ")
        if Units[i] == "T_r":
            TransitionNames = [chemicalDatsInterest[i][:-4] + "_" + x + "_T_r" for x in Transitions]
        elif Units[i] == "Intensity":
            TransitionNames = [chemicalDatsInterest[i][:-4] + "_" + x + "_Intensity" for x in Transitions]
        else:
            TransitionNames = [chemicalDatsInterest[i][:-4] + "_" + x + "_Flux" for x in Transitions]
        LinesInSql = np.append(LinesInSql, TransitionNames)
    Intensities = retrieveLinesOfEntry(MatchedID, LinesInSql, DBlocation, chemicalDatsInterest)
    return Intensities


def retrieveIntensitiesUI(ParameterDict, ChangingParams, DBlocation, LinesOfInterest, Chemicals, LinesGiven):
    ChangingParamsKeys = list(ChangingParams.keys())
    ChangingParamsValues = list(ChangingParams.values())
    MatchedID = []
    while len(MatchedID) < 1:
        MatchedID = checkAndRetrieveModel(ParameterDict=ParameterDict, ChangingParamsKeys=ChangingParamsKeys,
                                          ChangingParamsValues=ChangingParamsValues, DBlocation=DBlocation)
    ChemsAndLines = {}
        ...
        for j in range(len(LinesOfInterest)):
            if len(Lines[UniqueChems[i]]) <= 1000:
                ...
                if ChemTable in ChemsAndLines.keys():
                    ...
                    ChemsAndLines[ChemTable] = [LinesOfInterest[j]]
    Intensities = retrieveLinesOfEntry(MatchedID, LinesOfInterest, DBlocation, ChemsAndLines)
    return Intensities


def checkAndRetrieveModel(ParameterDict, ChangingParamsKeys, ChangingParamsValues,
                          DBlocation):
    outSpecies = list(Lines.keys())
    for i in range(len(ChangingParamsKeys)):
        ...
        ParameterDict[ChangingParamsKeys[i]] = ChangingParamsValues[i]
    sqlSearch = sqlSearch + ';'
    CurrentPID = os.getpid()
    mcf.Queue.put(("Search", [DBlocation, sqlSearch, CurrentPID]))
    SearchResults = RetrieveQueueResults(CurrentPID)
    if len(SearchResults) < 1 or SearchResults.values[0, 0] != None:
        ...
        if len(SearchResults) < 1:
            ParamDF, ChemDF = UCLChemDataFrames(ParameterDict, " ".join(outSpecies))
            UCLParamOut = RadexForGrid(UCLChemDict=ParameterDict, UCLParamDF=ParamDF,
                                       UCLChemDF=ChemDF)
            time.sleep(sleepTime)
            mcf.Queue.put(("Save", [UCLParamOut, ParameterDict, " ".join(outSpecies), DBlocation, CurrentPID]))
...
def PDSearchFunction(DBlocation, Search, PID):
    ...
    mcf.ResultDict[str(PID)] = Results


def createStartingPoints(LinesArray, ChangingParameterKeys, DistanceMethod, DBlocation, Walkers, SpreadPercent=0.9):
    ID = retrieveClosestMatches(LinesArray, DistanceMethod, DBlocation)
    sqlSearch = 'SELECT ' + (
...
def createGausStartingPoints(LinesArray, ChangingParameterKeys, DistanceMethod, DBlocation, Walkers, SpreadPercent=0.9):
    ID = retrieveClosestMatches(LinesArray, DistanceMethod, DBlocation)
    sqlSearch = 'SELECT ' + (
        str(ChangingParameterKeys)[1:-1].replace('\'', '"')) + ' FROM savedModels WHERE ModelID in (' + str(
        ID.tolist())[1:-1] + ');'
...
def createGausStartingPointsGrid(LinesArray, ChangingParameterKeys, DBlocation, Walkers, GridDictionary,
...
def worker_main():
    FuncDict = {"Search": PDSearchFunction, "Save": saveModel, "Stop": StopManager, "SaveComplete": PDSearchFunction}
#                "RunRADEX": FortranRADEX, "RunUCLCHEM": FortranUCLCHEM}
    while BoolDict["Running"]:
        f, args = mcf.Queue.get()
        FuncDict[f](*args)
...
def RetrieveQueueResults(CurrentProcess):
    DictKeys = mcf.ResultDict.keys()
    while str(CurrentProcess) not in DictKeys:
        DictKeys = mcf.ResultDict.keys()
    Results = mcf.ResultDict[str(CurrentProcess)]
    del mcf.ResultDict[str(CurrentProcess)]
    return Results
...
def RetrieveFortranQueueResults(CurrentProcess):
    DictKeys = mcf.FortranResultDict.keys()
    while str(CurrentProcess) not in DictKeys:
        DictKeys = mcf.FortranResultDict.keys()
